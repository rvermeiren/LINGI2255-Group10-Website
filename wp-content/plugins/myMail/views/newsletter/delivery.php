...
<ul class="category-tabs">
	<li class="<?php if ( !$is_autoresponder ) {
			echo 'tabs';
		}
	?>"><a href="#regular-campaign"><?php _e( 'Regular Campaign', 'mymail' );?></a></li>
	<li class="<?php if ( $is_autoresponder ) {
	echo 'tabs';
}
?>"><a href="#autoresponder"><?php _e( 'Auto Responder', 'mymail' );?></a></li>
</ul>
<div id="regular-campaign" class="tabs-panel"<?php if ( $is_autoresponder ) {
...
<div class="active_wrap <?php if ( $this->post_data['timezone'] ) {
		echo ' timezone-enabled';
	}
if ( $this->post_data['active'] && !$is_autoresponder ) {
	echo ' disabled';
}
?>">
...
<div id="autoresponder" class="tabs-panel"<?php if ( !$is_autoresponder ) {
		echo ' style="display:none"';
	}
?>>
	<?php
...
?>
	...
	<div id="autoresponder_wrap" class="autoresponder-<?php echo $autoresponderdata['action'] ?>">
		<div class="autoresponder_active_wrap<?php if ( $this->post_data['active'] && $is_autoresponder ) {
	echo ' disabled';
}
?>">
...
?>
		...
		<div class="mymail_autoresponder_more autoresponderfield-mymail_autoresponder_timebased<?php if ( ( $this->post_data['timezone'] ) ) {
	echo ' timezone-enabled';
}
?>">
			<p>
...
?>
		...
		<?php endif;?>
			...
			<p>
				...
				<div class="mymail_autoresponder_timebased-end-schedule-field" <?php if ( !$autoresponderdata['endschedule'] ) {
	echo ' style="display:none"';
}
?>>
<?php
...
?>
			...
			<div id="autoresponderfield-mymail_timebased_advanced"<?php if ( !isset( $autoresponderdata['time_conditions'] ) ) {
	echo ' style="display:none"';
}
?>>
				<p>
...
?>
		...
		<div class="mymail_autoresponder_more autoresponderfield-mymail_post_published <?php if ( isset( $autoresponderdata['time_conditions'] ) ) {
	echo ' autoresponderfield-mymail_autoresponder_timebased';
}
?>">
				<p class="description">
				<?php
...
?>
			...
			<p id="userexactdate">
				...
				<span <?php if ( $autoresponderdata['userexactdate'] ) {
		echo ' class="disabled"';
	}
	?>><?php echo sprintf( __( 'every %1$s %2$s', 'mymail' ), $amount, $unit ) ?></span></label><br>
	...
	?>><?php _e( 'on the exact date', 'mymail' );?></span></label>
			</p>
			<p>
				<?php
