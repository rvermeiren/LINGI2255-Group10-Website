			...
			<?php if ( !empty( $addon->wpslug ) ): ?>
				...
				<?php else: ?>
					<?php if ( current_user_can( 'install_plugins' ) || current_user_can( 'update_plugins' ) ): ?>
						<a class="button button-primary" href="<?php echo wp_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=' . dirname( $addon->wpslug ) . '&mymail-addon' ), 'install-plugin_' . dirname( $addon->wpslug ) ); ?>"><?php _e( 'Install', 'mymail' );?></a>
					<?php endif;?>
				<?php endif;?>

			<?php else: ?>

					<a class="button button-primary" href="<?php echo $addon->link ?>"><?php _e( 'Purchase', 'mymail' );?></a>
