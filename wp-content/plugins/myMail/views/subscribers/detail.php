...
if ( !$is_new ) {
	...
	if ( !current_user_can( 'mymail_add_subscribers' ) ) {
		wp_die( __( "You don't have the right permission to add new subscribers", 'mymail' ) );
	}
...
<table class="form-table">
	<tr>
		<td scope="row" class="avatar-wrap">
			<div class="avatar<?php if ( $subscriber->wp_id ) {
	echo ' wp-user';
}
?>" title="<?php _e( 'Source', 'mymail' )?>: Gravatar.com" style="background-image:url(<?php echo $this->get_gravatar_uri( $subscriber->email, 400 ); ?>)"></div>
		...
		<td class="user-info">
			...
			<div class="detail">
				...
				<ul class="click-to-edit type-name">
					...
					<li>
				<?php if ( mymail_option( 'name_order' ) ): ?>
		...
		<td class="user-meta" align="right">
			<?php if ( !$is_new ): ?>
				<?php if ( $meta->coords ): $geo = explode( '|', $meta->geo );?>
					...
					<p class="alignright">
						<?php if ( $geo[1] ) {
		echo __( 'from', 'mymail' ) . sprintf( ' %s, %s', '<strong><a href="https://www.google.com/maps/@' . $meta->coords . ',11z" class="external">' . $geo[1] . '</a></strong>', '<span class="mymail-flag-24 flag-' . strtolower( $geo[0] ) . '"></span> ' . mymail( 'geo' )->code2Country( $geo[0] ) );
	}
...
?>
			<h3><?php _e( 'Activity', 'mymail' );?></h3>
				<p>
				<?php if ( $open_time ): ?>
				<?php echo sprintf( __( '%1$s needs about %2$s to open a campaign', 'mymail' ), ( $subscriber->fullname ? $subscriber->fullname : __( 'User', 'mymail' ) ), '<strong>' . human_time_diff( $now + $open_time ) . '</strong>' ); ?> <?php if ( $click_time ) {
	echo sprintf( __( 'and %1$s to click a link', 'mymail' ), '<strong>' . human_time_diff( $now + $click_time ) . '</strong>' );
}
?>
				<?php else:
