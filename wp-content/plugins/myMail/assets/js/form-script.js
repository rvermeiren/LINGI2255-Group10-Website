jQuery(document).ready(function($) {
	...
	$('#use-it')
	.on('click', function(){
		tb_show(mymailL10n.useit, '#TB_inline?&width='+(Math.min(1200, _win.width()-50))+'&height='+(_win.height()-100)+'&inlineId=useitbox', null);
		if(saverequired) alert(mymailL10n.not_saved);
		return false;
	});
	...
	function requireSave(){
		...
		window.onbeforeunload = function(){
			return mymailL10n.require_save;
		};
	}
	...
	iframe.on('load', function(){
		_style = $('<style id="mymail_form_preview_css"></style>').appendTo(iframe.contents().find('head'));
		updateStyle();


		iframe.contents().find('style.mymail-custom-form-css').remove();

	});

	addcustomstyle.on('click', function(){
		var el = $(this).parent().find('.color-field'),
			style = '.mymail-form-'+ID+' '+el.data('selector')+'{\n    \n}\n';
	...
	customstylesamples.find('option').on('click', function(){
		customstyle.val(customstyle.val()+customstyleprefix.val()+'.mymail-form-'+ID+$(this).val()+"{\n    \n}\n");
	...
	.on('change', '.mymail_userschoice', function(){
		...
		$(this).parent().parent().parent().parent().find('legend.mymail_userschoice_td').hide().eq(checked ? 1 : 0).show();
	...
	function updateStyle(){
		...
		$.each(selectors, function(s, p){
			...
			style += 'form.mymail-form.mymail-form-'+ID+' '+s+'{\n';
			$.each(p, function(k,v){
				style += '\t'+k+': '+v+';\n';
			});
			style += '}\n';
	...
	buttoncode.on('click', function(){$(this).select()});
	...
	function updateButton(){
		...
		var code = window.mymaildata.embedcode,
			id = $('#ID').val(),
		...
		if(window.MyMailSubscribe && window.MyMailSubscribe.loaded) window.MyMailSubscribe.destroy();
		buttonpreview.html('');
