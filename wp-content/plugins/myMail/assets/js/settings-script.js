jQuery(document).ready(function($) {
	...
	$('form#mymail-settings-form').on('submit', function(){return false});
	if($('#settingsloaded').length){
		$('.submit-form').prop('disabled', false);
		$('form#mymail-settings-form').off('submit');
	}else{
	...
	deliverynav.on('click', 'a.nav-tab', function(){
		...
		$('input.mymail_sendtest').val(mymailL10n.save_to_test).prop('disabled', true);
		return false;
	});
	...
	nav.on('click', 'a', function(){
		...
		location.hash = hash;
		$('form#mymail-settings-form').attr('action','options.php'+hash);
		if(hash == '#system_info'){
			var textarea = $('#system_info_content');
			if($.trim(textarea.val())) return;
			textarea.val(mymailL10n.loading+'...');
	...
	if(location.hash == '#forms' && confirm('The forms are no longer on the settings page. Go there?')){
		location.href = 'edit.php?post_type=newsletter&page=mymail_forms';
	}
	...
	$('.mymail_sendtest').on('click', function(){
		var $this = $(this),
			...
			formdata = $('form#mymail-settings-form').serialize();
	...
	function bounce_test_check(identifier, count, formdata, callback){
		...
		_ajax('bounce_test_check',{identifier: identifier, passes: count, formdata: formdata}, function(response){
