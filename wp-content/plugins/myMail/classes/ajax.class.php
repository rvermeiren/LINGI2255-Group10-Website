...
class MyMailAjax {
		...
		if ( !$editorstyle ) {
			...
			if ( 0 != $current_user->ID ) {
				...
				$placeholder->add( array(
						...
						'fullname' => mymail_option( 'name_order' ) ? trim( $current_user->user_lastname . ' ' . $firstname ) : trim( $firstname . ' ' . $current_user->user_lastname ),
					) );
			}
	...
	private function create_new_template() {
		...
		if ( !$return['success'] ) {
			$return['msg'] = __( 'Unable to save template!', 'mymail' );
		}
	...
	private function send_test() {
		...
		if ( isset( $_POST['test'] ) ) {
			...
			if ( isset( $_POST['formdata'] ) ) {
				...
				mymail_update_option( $formdata['mymail_options'], true );
			}
			...
			$preheader = stripslashes( ( $_POST['preheader'] ) );
		...
		if ( is_numeric( $_POST['id'] ) ) {
			...
			if ( $post ) {
				...
				if ( !empty( $post->post_excerpt ) ) {
					...
					//prevent wp_trim_words from stripping html tags
					$excerpt = htmlentities( $content );
					$excerpt = wp_trim_words( $excerpt );
					$excerpt = html_entity_decode( $excerpt );
					//add a closing </p> if one is missing
					if ( substr_count( $excerpt, '<p' ) == substr_count( $excerpt, '</p>' ) + 1 ) {
						$excerpt .= '</p>';
					}
				}
				...
				$data = apply_filters( 'mymail_auto_post', array(
			...
			if ( !is_wp_error( $rss ) ) {
				...
				$data = apply_filters( 'mymail_auto_rss', array(
	...
	private function set_template_html() {
		...
		if ( $new ) {
			...
			$filename = mymail( 'template', $return['slug'] )->create_new( $name, $content );

		} else {

			global $wp_filesystem;
			mymail_require_filesystem();
			$path = mymail( 'templates', $return['slug'] )->get_path();
	...
	private function bounce_test() {
		...
		$mail->add_header( 'X-MyMail-Bounce-Identifier', $identifier );
	...
	private function bounce_test_check() {
		...
		$port = mymail_option( 'bounce_port', 110 );
		...
		if ( !$pop3->connect( $server, $port ) ) {
			$return['complete'] = true;
			$return['msg'] = sprintf( __( 'Unable to connect to bounce server %1$s on port %2$d! Error: %3$s', 'mymail' ), $server, $port, $pop3->ERROR );
			$this->json_return( $return );
			exit;
		}
		...
		if ( false === $count ) {
			$return['complete'] = true;
			$return['msg'] = sprintf( __( 'Unable to login with the given credentials! Error: %1$s', 'mymail' ), $pop3->ERROR );
			$this->json_return( $return );
			exit;
		}
