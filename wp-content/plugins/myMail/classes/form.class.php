...
class MyMailForm {
	...
	public function id( $id ) {
		...
		if ( !$this->form ) {
			$this->form = $this->form = mymail( 'forms' )->get( 1, true, true );
		}
	...
	public function render( $echo = true ) {
		...
		foreach ( $this->form->fields as $field ) {
			...
			case 'email':
				...
				if ( !$inline ) {
					$fields['email'] .= '<label for="mymail-email-' . $this->ID . '">' . $label . ' ' . ( $asterisk ? '<span class="mymail-required">*</span>' : '' ) . '</label>';
				}
			...
			case 'lastname':
				...
				if ( !$inline ) {
					$fields[$field->field_id] .= '<label for="mymail-' . $field->field_id . '-' . $this->ID . '">' . $label . ( $required && $asterisk ? ' <span class="mymail-required">*</span>' : '' ) . '</label>';
				}
			...
			default:
				...
				if ( $showlabel ) {
					...
					if ( $required && $asterisk ) {
						$fields[$field->field_id] .= ' <span class="mymail-required">*</span>';
					}
	...
	private function get_hash() {
		...
		if ( isset( $_COOKIE['mymail'] ) ) {
			return $_COOKIE['mymail'];
		}
	...
	private function unsubscribe_form() {
		...
		$html .= '<form action="' . $this->get_form_action( $action ) . '" method="post" class="mymail-form mymail-form-' . $form_id . ' mymail-form-submit mymail-ajax-form" id="mymail-form-unsubscribe">' . "\n";
