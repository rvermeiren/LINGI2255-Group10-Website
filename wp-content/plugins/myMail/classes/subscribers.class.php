...
class MyMailSubscribers {
	...
	public function init() {
		...
		add_action( 'mymail_subscriber_notification', array( &$this, 'subscriber_delayed_notification' ) );
		add_action( 'mymail_unsubscribe_notification', array( &$this, 'subscriber_delayed_unsubscribe_notification' ) );
...
?>
	...
	public function save_screen_options( $status, $option, $value ) {
		...
		if ( 'mymail_subscribers_per_page' == $option ) {
			return $value;
		}
	...
	public function update( $entry, $overwrite = true, $merge = false, $subscriber_notification = false ) {
		...
		if ( isset( $entry['email'] ) && !mymail_is_email( $entry['email'] ) ) {
			return new WP_Error( 'invalid_email', __( 'invalid email address', 'mymail' ) );
		}
		...
		if ( false !== $wpdb->query( $sql ) ) {
			...
			if ( !$bulkimport ) {
				mymail_cache_delete( 'subscribers' );
			}
	...
	public function add_from_wp_user( $user_id, $userdata = array() ) {
		...
		if ( !$user ) {
			new WP_Error( 'no_user', __( 'User does not exist!', 'mymail' ) );
		}
		...
		if ( $subscriber_exists ) {
			new WP_Error( 'subscriber_exists', __( 'Subscriber already exists', 'mymail' ) );
		}
	...
	public function wp_id( $ids = null ) {
		...
		if ( is_array( $ids ) ) {
			...
			foreach ( $ids as $id ) {
				$sql = "UPDATE {$wpdb->prefix}mymail_subscribers AS a LEFT JOIN {$wpdb->users} AS b ON a.email = b.user_email SET a.wp_id = b.ID WHERE a.ID = %d";
		...
		$sql = "UPDATE {$wpdb->prefix}mymail_subscribers AS a INNER JOIN {$wpdb->users} AS b ON a.email = b.user_email SET a.wp_id = b.ID";
	...
	public function get_count_by_status( $status = null ) {
		...
		if ( false === ( $counts = mymail_cache_get( 'get_count_by_status' ) ) ) {
			...
			$sql = "SELECT status, COUNT( * ) AS count FROM {$wpdb->prefix}mymail_subscribers AS a GROUP BY status";
	...
	public function subscriber_notification( $id, $timestamp = null ) {
		...
		if ( defined( 'MYMAIL_DO_BULKIMPORT' ) && MYMAIL_DO_BULKIMPORT ) {
			return false;
		}
		...
		if ( $delay = mymail_option( 'subscriber_notification_delay' ) ) {

			$timestamp = mymail( 'helper' )->get_timestamp_by_string( $delay );
			$timestamp = apply_filters( 'mymail_subscriber_notification_delay', $timestamp );

			return ( !wp_next_scheduled( 'mymail_subscriber_notification' ) )
				? wp_schedule_single_event( $timestamp, 'mymail_subscriber_notification' )
				: false;
	...
	public function subscriber_delayed_notification() {
		...
		if ( !mymail_option( 'subscriber_notification' ) || !mymail_option( 'subscriber_notification_receviers' ) || !mymail_option( 'subscriber_notification_delay' ) ) {
			return false;
		}

		return mymail( 'notification' )->add( time(), array(
	...
	public function subscriber_unsubscribe_notification( $id, $timestamp = null ) {
		...
		if ( !mymail_option( 'unsubscribe_notification' ) || !mymail_option( 'unsubscribe_notification_receviers' ) ) {
			return false;
		}

		if ( $delay = mymail_option( 'unsubscribe_notification_delay' ) ) {

			$timestamp = mymail( 'helper' )->get_timestamp_by_string( $delay );
			$timestamp = apply_filters( 'mymail_subscriber_unsubscribe_notification_delay', $timestamp );

			return ( !wp_next_scheduled( 'mymail_unsubscribe_notification' ) )
				? wp_schedule_single_event( $timestamp, 'mymail_unsubscribe_notification' )
				: false;
	...
	public function subscriber_delayed_unsubscribe_notification() {
		...
		if ( !mymail_option( 'unsubscribe_notification' ) || !mymail_option( 'unsubscribe_notification_receviers' ) || !mymail_option( 'unsubscribe_notification_delay' ) ) {
			return false;
		}
	...
	public function get( $ID = null, $custom_fields = false ) {
		...
		if ( $custom_fields ) {
			...
			$fullnamestring = mymail_option( 'name_order' )
			...
			$sql .= ", firstname.meta_value AS firstname, lastname.meta_value AS lastname, $fullnamestring as fullname";
		}
		...
		$sql .= $wpdb->prepare( " ORDER BY %s %s LIMIT %d, %d", $orderby, $order, $offset, $limit );
	...
	private function get_by_type( $type, $value, $custom_fields = false ) {
		...
		if ( $custom_fields ) {
			mymail_cache_set( 'subscribers', $cache );
		}
	...
	public function get_recipient_detail( $id, $campaign_id ) {
		...
		if ( $meta['client'] ) {
			$html .= sprintf( __( 'with %s', 'mymail' ), '<strong>' . $meta['client'] . '</strong>' );
		}

		if ( $meta['geo'] ):
			$geo = explode( '|', $meta['geo'] );
		if ( $geo[1] ) {
			$html .= ' ' . sprintf( _x( 'in %s, %s', 'in [city], [country]', 'mymail' ), '<strong>' . $geo[1] . '</strong>', '<span class="mymail-flag flag-' . strtolower( $geo[0] ) . '"></span> <strong>' . $geo[0] . '</strong>' );
		}
		...
		$html .= '</li>';
		if ( $actions->bounces ) {
			$html .= '<li><label class="red">' . sprintf( _n( '%s soft bounce', '%s soft bounces', $actions->softbounces_total, 'mymail' ), $actions->softbounces_total ) . '</label> <strong class="red">' . sprintf( __( 'Hard bounced at %s', 'mymail' ), date( $timeformat, $actions->bounces + $timeoffset ) . ', ' . sprintf( __( '%s ago', 'mymail' ), human_time_diff( $actions->bounces ) ) ) . '</strong> </li>';
		}
	...
	public function user_register( $user_id ) {
		...
		if ( !apply_filters( 'mymail_user_register', true ) ) {
			return;
		}
		...
		if ( $is_register ) {
			...
			if ( !mymail_option( 'register_signup' ) ) {
				return false;
			}

			//stop if not option
			if ( !isset( $_POST['mymail_user_newsletter_signup'] ) ) {
				return false;
			}
			...
			if ( !mymail_option( 'register_other' ) ) {
				return false;
			}
	...
	public function comment_form_checkbox() {
		...
		if ( !mymail_option( 'register_comment_form' ) ) {
			return;
		}
		...
		if ( isset( $_POST['newsletter_signup'] ) ) {
			...
			if ( in_array( $comment_approved . '', mymail_option( 'register_comment_form_status', array() ) ) ) {
				...
				if ( $comment && !$this->get_by_mail( $comment->comment_author_email ) ) {
					...
					$lists = mymail_option( 'register_comment_form_lists', array() );
					$lists = apply_filters( 'mymail_comment_post_lists', $lists, $comment, $comment_approved );
	...
	public function wp_set_comment_status( $comment_id, $comment_status ) {
		...
		if ( !mymail_option( 'register_comment_form' ) || !in_array( $comment_status . '', array( '1', 'approve' ) ) ) {
			return false;
		}
