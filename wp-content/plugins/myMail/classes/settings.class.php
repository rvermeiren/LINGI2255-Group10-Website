...
class MyMailSettings {
	...
	public function get_defaults() {
		...
		$host = !mymail_is_local() ? $_SERVER['HTTP_HOST'] : '';
	...
	public function scripts_styles() {
		...
		wp_localize_script( 'mymail-settings-script', 'mymailL10n', array(
				...
				'save_to_test' => __( 'Save to Test', 'mymail' ),
	...
	public function verify( $options ) {
		...
		if ( isset( $_POST['mymail_generate_dkim_keys'] ) ) {
			...
			try {
				...
				add_settings_error( 'mymail_options', 'mymail_options', __( 'Not able to create new DKIM keys!', 'mymail' ) . '<br>' . $e->getMessage() );
		...
		foreach ( $options as $id => $value ) {
			...
			case 'trackcountries':
				...
				if ( empty( $options['countries_db'] ) ) {
					$options['countries_db'] = MYMAIL_UPLOAD_DIR . '/GeoIPv6.dat';
				}
			...
			case 'trackcities':
				...
				if ( empty( $options['cities_db'] ) ) {
					$options['cities_db'] = MYMAIL_UPLOAD_DIR . '/GeoIPCity.dat';
				}
			...
			case 'archive_slug':
				...
				if ( $old != $value ) {
					if ( $options['hasarchive'] ) {
						add_settings_error( 'mymail_options', 'mymail_options', sprintf( __( 'Your newsletter archive page is: %s', 'mymail' ), '<a href="' . home_url( $value ) . '" class="external">' . home_url( $value ) . '</a>' ) );
					}
			...
			case 'cron_lock':
				...
				if ( $old != $value ) {
					...
					switch ( $old ) {
					case 'file':
						$lockfiles = glob( MYMAIL_UPLOAD_DIR . '/CRON_*.lockfile' );
						foreach ( $lockfiles as $lockfile ) {
							@unlink( $lockfile );
						}
						break;
					case 'db':
						$sql = "DELETE FROM {$wpdb->options} WHERE option_name LIKE 'mymail_cron_lock_%'";
						$wpdb->query( $sql );
						break;
			...
			case 'subscriber_notification_delay':
				...
				if ( $old != $value ) {
					...
					if ( wp_next_scheduled( 'mymail_subscriber_notification' ) ) {
						wp_clear_scheduled_hook( 'mymail_subscriber_notification' );
					}

					if ( $value ) {

						$timestamp = mymail( 'helper' )->get_timestamp_by_string( $value );
			...
			case 'unsubscribe_notification_delay':
				...
				if ( $old != $value ) {
					...
					if ( wp_next_scheduled( 'mymail_unsubscribe_notification' ) ) {
						wp_clear_scheduled_hook( 'mymail_unsubscribe_notification' );
					}
			...
			case 'send_at_once':
				...
				if ( $old != $value ) {
					...
					$value = max( $value, 1 );
					if ( $value >= 300 ) {
						add_settings_error( 'mymail_options', 'mymail_options', sprintf( __( 'sending %s emails at once can cause problems with statistics cause of a server timeout or to much memory usage! You should decrease it if you have problems!', 'mymail' ), $value ) );
					}
			...
			case 'dkim':
				...
				if ( !isset( $options['dkim_private_key'] ) || !isset( $options['dkim_public_key'] ) ) {
					add_settings_error( 'mymail_options', 'mymail_options', __( 'You have to generate DKIM Keys to use DKIM signed mails!', 'mymail' ) );
				}
