...
class MyMailPlaceholder {
	...
	public function get_social_service( $service, $url, $title = '', $fallback = '' ) {
		...
		if ( !$this->social_services ) {
			$this->social_services = mymail( 'helper' )->social_services();
		}
			...
			for ( $i = 0; $i < $count; $i++ ) {
				...
				if ( $relative_to_absolute ) {
					...
					$replace_to = '{' . $post_type . '_' . $hits[3][$i] . ':' . $post->ID . '}';
						...
						case 'excerpt':
							if ( !empty( $post->{'post_excerpt'} ) ) {
								$replace_to = $post->{'post_excerpt'};
							} else {
								//prevent wp_trim_words from stripping html tags
								$replace_to = htmlentities( $post->{'post_content'} );
								$replace_to = wp_trim_words( $replace_to );
								$replace_to = html_entity_decode( $replace_to );
								//add a closing </p> if one is missing
								if ( substr_count( $replace_to, '<p' ) == substr_count( $replace_to, '</p>' ) + 1 ) {
									$replace_to .= '</p>';
								}
							}
							break;
