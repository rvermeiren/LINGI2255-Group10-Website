...
class MyMailActions {
	...
	private function add_subscriber_action( $args, $explicit = true ) {
		...
		if ( mymail_option( 'do_not_track' ) && isset( $_SERVER['HTTP_DNT'] ) && $_SERVER['HTTP_DNT'] == 1 ) {
			return;
		}
	...
	private function add( $args, $explicit = true ) {
		...
		if ( false !== $wpdb->query( $sql ) ) {
			if ( $args['type'] != 1 && $explicit ) {
				mymail( 'subscribers' )->update_rate( $args['subscriber_id'] );
			}
	...
	public function get_by_campaign( $campaign_id = null, $action = null, $strict = false ) {
		...
		$action_counts = mymail_cache_get( $cache_key );
		if ( !$action_counts ) {
			$action_counts = array();
		}
		...
		//$sql = "SELECT a.campaign_id AS ID, type, IF(a.subscriber_id, COUNT(DISTINCT a.subscriber_id), SUM(a.count)) AS count, SUM(a.count) AS total FROM {$wpdb->prefix}mymail_actions AS a";
		...
		mymail_cache_set( $cache_key, $action_counts );
	...
	public function get_by_subscriber( $subscriber_id = null, $action = null, $strict = false ) {
		...
		//$sql = "SELECT a.campaign_id, a.subscriber_id AS ID, type, IF(a.subscriber_id, COUNT(DISTINCT a.subscriber_id), SUM(a.count)) AS count, SUM(a.count) AS total FROM {$wpdb->prefix}mymail_actions AS a";
	...
	public function get_dashboard_actions( $scale = 'days', $limit = 7, $offset = 0, $sets = null ) {
		...
		$sql .= ") t1 LEFT JOIN {$wpdb->prefix}mymail_actions AS a ON FROM_UNIXTIME(a.timestamp+$timeoffset, '%Y-%m-%d') = t1.date";
	...
	public function get_chronological_actions( $scale = 'days', $since = null, $desc = true ) {
		...
		if ( false === ( $actions = mymail_cache_get( 'chronological_actions_' . $scale . $since . $desc ) ) ) {
			...
			mymail_cache_add( 'chronological_actions_' . $scale . $since . $desc, $actions );
	...
	public function get_campaign_actions( $campaign_id, $subscriber_id = null, $action = null, $cache = true ) {
		...
		if ( false === ( $actions = mymail_cache_get( 'campaign_actions' ) ) ) {
			...
			if ( $cache ) {
				mymail_cache_add( 'campaign_actions', $actions );
			}
	...
	public function get_clicked_links( $campaign_id ) {
		...
		if ( false === ( $clicked_links = mymail_cache_get( 'clicked_links_' . $campaign_id ) ) ) {
			...
			mymail_cache_add( 'clicked_links_' . $campaign_id, $clicked_links );
	...
	public function get_clients( $campaign_id ) {
		...
		if ( false === ( $clients = mymail_cache_get( 'clients_' . $campaign_id ) ) ) {
			...
			mymail_cache_add( 'clients_' . $campaign_id, $clients );
	...
	public function get_environment( $campaign_id ) {
		...
		if ( false === ( $environment = mymail_cache_get( 'environment_' . $campaign_id ) ) ) {
			...
			mymail_cache_add( 'environment_' . $campaign_id, $environment );
	...
	public function get_error_list( $campaign_id ) {
		...
		if ( false === ( $error_list = mymail_cache_get( 'error_list_' . $campaign_id ) ) ) {
			...
			mymail_cache_add( 'error_list_' . $campaign_id, $error_list );
