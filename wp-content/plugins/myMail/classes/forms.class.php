...
class MyMailForms {
	...
	public function form_head() {
		...
		wp_register_style( 'mymail-form-default-style', MYMAIL_URI . 'assets/css/form-default-style' . $suffix . '.css', array(), MYMAIL_VERSION );

		if ( $use_style ) {
			wp_register_style( 'mymail-theme-style', get_template_directory_uri() . '/style.css', array(), MYMAIL_VERSION );
			wp_print_styles( 'mymail-theme-style' );
		}
		if ( $is_button ) {
			...
			wp_register_style( 'mymail-form-button-base-style', MYMAIL_URI . 'assets/css/button-style' . $suffix . '.css', array(), MYMAIL_VERSION );
			wp_register_style( 'mymail-form-button-style', MYMAIL_URI . 'assets/css/button-' . $buttonstyle[0] . '-style' . $suffix . '.css', array( 'mymail-form-button-base-style' ), MYMAIL_VERSION );
			//wp_print_styles('form-button-style');
			mymail( 'helper' )->wp_print_embedded_styles( 'mymail-form-button-style' );

		} elseif ( $is_editable ) {

			wp_print_styles( 'mymail-form-default-style' );
			...
			wp_register_style( 'mymail-form-iframe-style', MYMAIL_URI . 'assets/css/form-iframe-style' . $suffix . '.css', array( 'mymail-form-default-style' ), MYMAIL_VERSION );
			mymail( 'helper' )->wp_print_embedded_styles( 'mymail-form-iframe-style' );
	...
	public function form_footer() {
		...
		if ( $is_button ) {
			...
			wp_register_script( 'mymail-form-button-script', MYMAIL_URI . 'assets/js/form-button-script' . $suffix . '.js', array(), MYMAIL_VERSION );
			//wp_localize_script( 'mymail-form-button-script', 'MyMailData', $MyMailData);
			mymail( 'helper' )->wp_print_embedded_scripts( 'mymail-form-button-script' );
			//wp_print_scripts('mymail-form-button-script');
			...
			wp_register_script( 'mymail-form-iframe-script', MYMAIL_URI . 'assets/js/form-iframe-script' . $suffix . '.js', array( 'jquery' ), MYMAIL_VERSION );
			//wp_localize_script('mymail-form-iframe-script', 'MyMailData', $MyMailData);
			wp_print_scripts( 'mymail-form-iframe-script' );
			wp_print_scripts( 'mymail-form' );
	...
	public function script_styles() {
		...
		wp_localize_script( 'mymail-form-detail', 'mymailL10n', array(
				...
				'not_saved' => __( 'You haven\'t saved your recent changes on this form!', 'mymail' ),
				'prev' => __( 'prev', 'mymail' ),
				'useit' => __( 'Use your form as', 'mymail' ) . '&hellip;',
			) );
		...
		if ( isset( $_POST['mymail_data'] ) ) {
			...
			if ( isset( $_POST['mymail_structure'] ) ) {
				...
				if ( !isset( $structure['fields']['email'] ) ) {
					$structure['fields']['email'] = mymail_text( 'email' );
				}
	...
	public function save_screen_options( $status, $option, $value ) {
		...
		if ( 'mymail_forms_per_page' == $option ) {
			return $value;
		}
	...
	public function get( $ID, $fields = true, $lists = true ) {
		...
		foreach ( $forms as $i => $form ) {
			...
			if ( empty( $forms[$i]->submit ) ) {
				$forms[$i]->submit = mymail_text( 'submitbutton' );
			}
