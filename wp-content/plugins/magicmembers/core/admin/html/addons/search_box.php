	<div class="table" id="addons-search-table">
		<div class="row">
		...
		<div class="row">
			...
			<div class="cell  ">
				...
				<input class="button" type="button" name="list_filter_btn" value="<?php _e('Show','mgm') ?>" onclick="mgm_addon_list_filter(true)" />
				<a href="javascript:mgm_addon_list()" title="<?php _e('Refresh List','mgm')?>"><img src="<?php echo MGM_ASSETS_URL ?>images/icons/arrow_refresh.png" /></a>
			</div>
		</div>
	</div>
	...
	<script language="javascript">
		jQuery(document).ready(function(){
			...
			jQuery("#addon_list select[name='search_field_name']").bind('change',function() {	
				// remove old		
				jQuery("#addon_list :input[name='search_field_value']").remove();		
				// reset val
				...
				switch(jQuery(this).val()){				
					case 'create_dt':
					case 'expire_dt':									
						jQuery('#addon_list #fld_wrapper').html('<input type="text" name="search_field_value" value="'+search_val+'" size="10">');
						if(!jQuery("#addon_list :input[name='search_field_value']").hasClass('hasDatepicker')){					
							mgm_date_picker("#addon_list :input[name='search_field_value']",'<?php echo MGM_ASSETS_URL?>', {yearRange:"<?php echo mgm_get_calendar_year_range(); ?>", dateFormat: "<?php echo mgm_get_datepicker_format()?>"});
						}
					break;
					default:					
						jQuery('#addon_list #fld_wrapper').html('<input type="text" name="search_field_value" value="'+search_val+'" size="20">');
					break;
				}
				onchange_count++;
			}).change();	
