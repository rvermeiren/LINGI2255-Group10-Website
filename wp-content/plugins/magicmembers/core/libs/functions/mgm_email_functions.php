...
function mgm_notify_user_expiration_reminder($user, $email_data){
	...
	// add filter
	$template = apply_filters('mgm_reminder_email_template', $template);

	// send mail
	return @mgm_mail($user->user_email, $template['subject'], $template['message'], 'expiration_reminder');		
}
...
function mgm_notify_admin_license_renewal_reminder() {	
	...
	$message .= "http://www.magicmembers.com/products-page/magic-members/magic-members-service-renewal/ <br/>";
