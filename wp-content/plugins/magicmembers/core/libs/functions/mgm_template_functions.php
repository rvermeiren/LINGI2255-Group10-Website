...
function mgm_user_subscription_info($user_id=NULL,$args=array()) {	
	...
	$html .= '
		...
		<div class="mgm_mmdetails_row ' . ($row_ctr++ % 2 == 0 ? 'alternate' : '') . '">
			...
			<div class="mgm_mmdetails_field_divider">:</div>
			<div class="mgm_mmdetails_field_description">' .((is_super_admin() ? 'N/A' : mgm_stripslashes_deep(esc_html(mgm_get_user_membership_type($user->ID)))) .' (<a href="' . apply_filters('mgm_upgrade_subscription_url', $upgrade_subscription_url, $usu_params) . '">' . __('Upgrade','mgm') . '</a>)').'</div>
		</div>
...
function mgm_upload_script_js($form_id='mgm-form-profile', $images, $field_type = null) {
	...
	// add
	$upload_url = add_query_arg(array('file_upload'=>'image','field_type'=>$field_type,'file_token'=>$file_token), $upload_url); 
	$delete_url = add_query_arg(array('file_unset'=>'image','field_type'=>$field_type,'file_token'=>$file_token), $delete_url); 
