...
class mgm_auth extends mgm_object{
	...
	function configure_service_properties(){
		...
		$this->set_service_property('product_service_domain',array('ini'=>'product_service_domain','const'=>'MGM_SERVICE_DOMAIN','default'=>'http://www.magicmembers.com/'));		
	...
	function get_request_info($license=false){
		// info
		$info = array('product_id'=>$this->product_id, 'product_name'=>$this->product_name, 'product_brand'=>$this->product_brand,
					  'version'=>$this->product_version, 'host'=>$this->product_install_host, 'build'=>$this->product_build, 
					  'stage'=>$this->product_stage);
	...
	function get_subscription_status(){		
		...
		/****************************************************************************************************************************
		$test_url = MGM_SERVICE_SITE . 'check_subscription&' . mgm_http_build_query(array('product_id'=>MGM_PRODUCT_ID,
		                                       'product_name'=>MGM_PRODUCT_NAME,'product_brand'=>MGM_PRODUCT_BRAND,
											   'version'=>MGM_VERSION,'build'=>MGM_BUILD,
											   'host'=>'http://speakingcpr.edrivenstudios.com'));
		// return $test_url;
		
		$subscription_status = mgm_remote_get($test_url);											   
	    *****************************************************************************************************************************/	
		...
		if(trim($subscription_status) == 'LOCKED' || trim($subscription_status) == 'EXPIRED'){		
			...
			// set
			$subscription_status  =  '<script language="javascript">window.location.href="admin-ajax.php?action=mgm_admin_ajax_action&page=mgm/admin";</script>';
