...
function mgm_register_post($sanitized_user_login = '', $user_email = '', $errors = null, $show_fields = null) {	
	...
	//check and append
	if(!empty($cf_register_by_membership_types)) $cf_register_page = array_merge($cf_register_by_membership_types,$cf_register_page);
...
function mgm_module_rebill_status($user_id, $member=NULL) {	
	...
	if( ! isset($module_obj->code) ){
		...
		// log
		mgm_log( sprintf('module [%s] not valid, exit rebill status check.', $module), __FUNCTION__);
		// return
		return false;
	}	
	...
	if( $member->status == MGM_STATUS_AWAITING_CANCEL ){
		...
		if( $time < strtotime(	$member->status_reset_on ) ){
			// str
			$log_str = sprintf('member status awaiting cancel and current date [%s] behind reset date [%s], exit rebill status check.', date('m-d-Y', $time), $member->status_reset_on);
			// log
			mgm_log( $log_str, __FUNCTION__);
			// return
			return false;	
	...
	if ( ! $module_obj->is_rebill_status_check_supported() ){
		// log
		mgm_log( sprintf('module [%s] does not support rebill status check, exit rebill status check.', $member->payment_info->module), __FUNCTION__);
		// return
		return false;	
	}	
...
function mgm_force_upgrade_if_freepack($member) {
	...
	if (($member->status == MGM_STATUS_EXPIRED || $member->status == MGM_STATUS_CANCELLED ) && $pack['cost'] == 0 && $pack['allow_renewal'] == 0){
		$action = 'upgrade';			
	}
	// if free trail pack paid or free and renewal not allowed, allow the user to upgrade
	if($pack['trial_on']) {
		if (($member->status == MGM_STATUS_EXPIRED || $member->status == MGM_STATUS_CANCELLED ) && $pack['allow_renewal'] == 0) {
			$action = 'upgrade';
		}	
	}
	// return
