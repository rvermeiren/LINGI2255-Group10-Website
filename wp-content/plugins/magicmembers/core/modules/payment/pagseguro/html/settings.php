...
<div id="module_settings_<?php echo $data['module']->code?>">
	<?php mgm_box_top($data['module']->name. ' Settings');?>
		...
		<div class="table">
			<div class="row">
				<div class="cell">
					<p><b><?php _e('PagSeguro Receiver Email','mgm'); ?>:</b></p>				
				</div>
			</div>		
			<div class="row">
				<div class="cell">
					<input type="text" name="setting[receiver_email]" id="setting_receiver_email" value="<?php echo esc_html($data['module']->setting['receiver_email']); ?>" size="50"/>
					<p><div class="tips"><?php _e('PagSeguro receiver email where the payment will be sent.','mgm'); ?></div></p>
				</div>
			</div>
			<div class="row">
				<div class="cell">
					<p><b><?php _e('PagSeguro API Token','mgm'); ?>:</b></p>
				</div>
			</div>
			<div class="row">
				<div class="cell">
					<input type="text" name="setting[token]" id="setting_token" value="<?php echo esc_html($data['module']->setting['token']); ?>" size="50"/>
					<p><div class="tips"><?php _e('PagSeguro API Token Provided by PagSeguro. Obtain it from your PagSeguro Account Payment Settings.','mgm'); ?></div></p>
				</div>
			</div>
			<div class="row">
				<div class="cell">
					<p><b><?php _e('PagSeguro Payment Via','mgm'); ?>:</b></p>
				</div>
			</div>
			<div class="row">
				<div class="cell">
					<select name="setting[payment_via]" id="setting_payment_via" class="width120px">
						<?php echo mgm_make_combo_options( array('html_form'=>__('HTML Form','mgm'),'api'=>__('API','mgm') ), $data['module']->setting['payment_via'], MGM_KEY_VALUE)?>
					</select>							
					<p><div class="tips"><?php _e('PagSeguro payment via option.','mgm'); ?></div></p>
				</div>
			</div>
			<div class="row api-options">
				<div class="cell">
			...
			<div class="row api-options">
				<div class="cell">
					<p><b><?php _e('PagSeguro Payment API Post Charset','mgm'); ?>:</b></p>
				</div>
			</div>
			<div class="row api-options">
				<div class="cell">
					<select name="setting[charset]" id="setting_charset" class="width120px">
						<?php echo mgm_make_combo_options( array('UTF-8'=>__('UTF-8','mgm'),'ISO-8859-1'=>__('ISO-8859-1','mgm') ), $data['module']->setting['charset'], MGM_KEY_VALUE)?>
					</select>							
					<p><div class="tips"><?php _e('PagSeguro payment api post data charset.','mgm'); ?></div></p>
				</div>
			</div>
			
			<div class="row api-options">
				<div class="cell">
					<p><b><?php _e('PagSeguro Transaction Code Parameter','mgm'); ?>:</b></p>
				</div>
			</div>
			<div class="row api-options">
				<div class="cell">
					<input type="text" name="setting[return_code]" id="setting_return_code" value="<?php echo esc_html($data['module']->setting['return_code']); ?>" size="50"/>
					<p><div class="tips"><?php _e('PagSeguro transaction code parameter sent to your redirect url. Match it with your PagSeguro Account Payment Settings -> Redirection with transaction code.','mgm'); ?></div></p>
				</div>
			</div>
			...
			<div class="row">
				<div class="cell">
					...
					<p><div class="tips"><?php _e('Currency to use, update primary currency in General Settings page. For Payment via API, Only "BRL" supported.','mgm'); ?></div></p>
				</div>
			</div>
			<?php /*?>
			<div class="row">
				<div class="cell">
					<p><b><?php _e('PagSeguro local site to use','mgm'); ?>:</b></p>
				</div>
			</div>
			<div class="row">
				<div class="cell">
					...
					<p><div class="tips"><?php _e('PagSeguro locale to use.','mgm'); ?></div></p>
				</div>
			</div>
			<?php */?>
			...
			<div class="row">
				<div class="cell">
					...
					<p><div class="tips"><?php _e('Notify URL for capturing silent post data sent from PagSeguro. READONLY, only for information. Please setup this URL in your PagSeguro Account IPN settings in order to use unsubscribe feature.','mgm'); ?></div></p>
				</div>
			</div>
			<div class="row">
				<div class="cell">
					<p><b><?php _e('Redirect URL','mgm'); ?>:</b></p>
				</div>
			</div>
			<div class="row">
				<div class="cell">
					<?php echo $data['module']->setting['return_url']?>												
					<p><div class="tips"><?php _e('Redirect URL for redirecting user after payment, to be set in PagSeguro Redirect Page. READONLY, only for information.','mgm'); ?></div></p>
				</div>
			</div>
			<div class="row">
				<div class="cell">
...
<script language="javascript">
	...
	jQuery(document).ready(function(){   
		...
		jQuery("#frmmod_<?php echo $data['module']->code?>").validate({
			...
			rules: {			
				'setting[receiver_email]': {required:true,email:true}
			},
			messages: {		
				'setting[receiver_email]': {required:"<?php _e('Please enter PagSeguro receiver email.','mgm')?>",email:"<?php _e('Please enter valid email.','mgm')?>"}
			},
			errorClass: 'invalid'
		});			
		// attach uploader
		...
		jQuery("#frmmod_<?php echo $data['module']->code?> :input[name='setting[payment_via]']").bind('change', function(){
			if( jQuery(this).val() == 'api' ){
				jQuery('.api-options').fadeIn('slow')
			}else{
				jQuery('.api-options').fadeOut('slow')
			}
		}).change();		
	});	
	//-->	
