...
class mgm_pagseguro extends mgm_payment{	
	...
	function process_return() {
		...
		if(count($_REQUEST)>3){
			...
			// log
			mgm_log($result, $this->module .'_'. __FUNCTION__);
			//check
	...
	function process_notify() {		
		...
		if($this->_verify_callback()){ // verify pagseguro payment data				
			...
			mgm_log('FROM PagSeguro process_notify: VERIFY Failed', $this->module .'_' . __FUNCTION__);	
		}
    ...
    */
	...
	function _setup_endpoints($end_points = array()){
		// define defaults
		$end_points_default = array('test'                 => 'https://sandbox.pagseguro.uol.com.br/v2/checkout/payment.html',
	...
	function _verify_callback(){
		...
		if( ! $dge ){
			...
			mgm_log('PagSeguro verification failed: ' . $message, $this->module .'_' . __FUNCTION__);
		}	
	...
	function _get_notification_data(){
		...
		if($notification_code){
			...
			// log xml as came			
			mgm_log('notification_url: ' . $notification_url,  $this->module .'_' . __FUNCTION__);
	...
	function _get_transaction_data( $transaction_code ){
		...
		if($transaction_code){
			...
			// log xml as came			
			mgm_log('transaction_url: ' . $transaction_url,  $this->module .'_' . __FUNCTION__);
			...
			// parse
				...
				// log
				mgm_log('PagSeguro Transaction : processed :'.print_r($_REQUEST, true), $this->module .'_' . __FUNCTION__);
			}			
		}
	}
	...
	function _get_autoreturn_notification_post() {		
		//mgm_log(mgm_array_dump($_POST,true),$this->module.'_'.__FUNCTION__);
		$post_data = 'Comando=validar&Token='.$this->setting['token'];
		// adding arguments		
		foreach ($_POST as $key => $value) {
			$post_data .= "&$key=" . $this->clearStr($value);
		}		
		// mgm_log($postdata,$this->module.'_'.__FUNCTION__);
	...
	function _curl_post($data) {
		...
		curl_close($curl);		
		//mgm_log(mgm_array_dump($result,true),$this->module.'_'.__FUNCTION__);
		return $result;
	}	
	...
	function check_transaction(){
		...
		if(isset($_REQUEST['transaction_id'])){	
			...
			//mgm_log('_pagseguro custom val : '.$custom_val,$this->module.'_'.__FUNCTION__);
			...
			//mgm_log('_pagseguro_data : '.mgm_array_dump($pagseguro_data,true),$this->module.'_'.__FUNCTION__);		
		}*/
		// code
		$return_code = !empty($this->setting['return_code']) ? $this->setting['return_code'] : 'transaction_id';
		// log
		mgm_log('return_code: '. $return_code, $this->module .'_' . __FUNCTION__);
		// check
