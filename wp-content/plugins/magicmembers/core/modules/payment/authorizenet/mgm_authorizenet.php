...
// -----------------------------------------------------------------------
/**
 * Authorize.Net payment module, integrates AIM and ARB
 ...
 * @version    3.0
 */
class mgm_authorizenet extends mgm_payment{
	// construct
	function __construct(){
		// php4 construct
		$this->mgm_authorizenet();
	}
	...
	function process_credit_card(){			
		...
		// log
		//mgm_log(mgm_filter_cc_details($post_data), __FUNCTION__);		
		// filter post data
	...
	function dependency_check(){
		...
		if(!extension_loaded('mhash')){
			$this->dependency[] = '<b class="mgm_module_dependency_high">MHASH PHP extension must be loaded for Authorize.Net.</b>';
		}
		******************************/
		if(!extension_loaded('SimpleXML')){
			$this->dependency[] = '<b class="mgm_module_dependency_high">'.__('SimpleXML PHP extension must be loaded for Authorize.Net ARB','mgm').'.</b>';
		}
		// transaction details api
		$this->dependency[] = '<b class="mgm_module_dependency_medium">'.__('Transaction Details API in Authorize.Net must be enabled for Rebill Status Query','mgm').'.</b>';
		// error		
	...
	function get_transaction_info($member, $date_format){		
		...
		// set default
		$authorizenet_txn_id  = __('N/A','mgm');
		// eway tran
		if(isset($member->payment_info->authorizenet_txn_id)){
			$authorizenet_txn_id = $member->payment_info->authorizenet_txn_id;
		}
		// info
	...
	function get_tracking_fields_html(){
		// html
		$html = sprintf('<p>%s: <input type="text" size="20" name="authorizenet[subscriber_id]"/></p>
				 		 <p>%s: <input type="text" size="20" name="authorizenet[transaction_id]"/></p>', 
	...
	function _buy_post() {
		...
		// passthrough
		$alt_tran_id = $this->_get_alternate_transaction_id('x_custom');
		...
		switch ($response_code) {
			case 'Approved':
				...
				// transaction id
				$transaction_id = $this->_get_transaction_id('x_custom');
				// hook args
			...
			if (isset($this->response['subscription_status'])) {
				...
				switch(strtolower($this->response['subscription_status'])){
					case 'active':
						...
						// expire on error
						if( isset($tran_response['response_status']) && ((int)$tran_response['response_status'] == 5 || (int)$tran_response['response_status'] == 2)){
							// set as expired
							$member->status = $new_status = MGM_STATUS_EXPIRED;
	...
	function get_transaction_details($member){
		...
		// check - issue #1678
		if ((isset($member->payment_info->txn_id) && !empty($member->payment_info->txn_id))||(isset($member->payment_info->authorizenet_txn_id) && !empty($member->payment_info->authorizenet_txn_id))) {
			if(empty($member->payment_info->txn_id)) {
				$member->payment_info->txn_id =$member->payment_info->authorizenet_txn_id;
			}
			// set xml content											
	...
	function _setup_endpoints($end_points = array()){
		// define defaults
		$end_points_default = array('test_sim' => 'https://test.authorize.net/gateway/transact.dll',
									'live_sim' => 'https://secure2.authorize.net/gateway/transact.dll',
									'test_aim' => 'https://test.authorize.net/gateway/transact.dll',
									'live_aim' => 'https://secure2.authorize.net/gateway/transact.dll',
									'test_arb' => 'https://apitest.authorize.net/xml/v1/request.api',
									'live_arb' => 'https://api2.authorize.net/xml/v1/request.api');	
		// merge
	...
	function _filter_postdata($gateway_method, $post_data, $return='string'){	
		...
		switch($gateway_method){
			case 'arb':
				...
				$content .=			"</customer>".
									"<billTo>".
										"<firstName>". ((isset($post_data['x_first_name'])) ? $post_data['x_first_name'] : $ch_first_name) . "</firstName>".
										"<lastName>" . ((isset($post_data['x_last_name'])) ? $post_data['x_last_name'] : $ch_last_name) . "</lastName>";
			...
			case 'aim':
				...
				$this->delim_char = '|';
				// custom set
				$aim_fields = array('x_version'        => '3.1', // 68 system fields
									'x_type'           => 'AUTH_CAPTURE',
				...
				$fields_sent = array('x_login', 'x_tran_key', 'x_description', 'x_first_name', 'x_last_name', 'x_email', 
									 'x_cust_id', 'x_amount', 'x_address', 'x_city', 'x_state', 'x_zip', 'x_country', 'x_phone');	
				                
				// first last name
				if((isset($post_data['x_first_name']) == '') || (isset($post_data['x_first_name'])==isset($post_data['x_last_name']))) {
					$post_data['x_first_name'] = $ch_first_name;
				}
				// last name
				if((isset($post_data['x_last_name']) == '') || (isset($post_data['x_first_name'])==isset($post_data['x_last_name']))) {
		...
		switch($gateway_method){
			case 'arb':
				if( $xml = @simplexml_load_string($content) ){
					...
					$temp_resp['ref_id'] 		  = (string)$xml->refId;						
				}else{
			...
			case 'aim':
				// split
				$data = $this->get_positional_fields($content);
				// log
				// mgm_log($data, __FUNCTION__);
				// error
				if(count($data)==0){
					$temp_resp['response_status'] = 3;
					$temp_resp['message_text']    = strip_tags($data[0]);
				}else{										
					// set 
			...
			case 'arb_silent_post':
				...
				if(count($content)==0){					
					...
					$fields = array(0 => "x_response_code", 1=>"x_response_subcode", 2=>"x_response_reason_code", 3=>"x_response_reason_text",
             						4 => "x_invoice_num", 5=>"x_trans_id", 6=>"x_transaction_id", 9=>"x_amount", 10 => "x_type", 
              						11 => "x_subscription_id");
					// store					
					...
					$temp_resp['transaction_id']   = (isset($x_transaction_id) && !empty($x_transaction_id)) ? $x_transaction_id : $x_trans_id;
			...
			case 'transaction_details':
				...
				if( $xml = @simplexml_load_string($content) ){
					...
					if ( isset($xml->messages->resultCode) && 'ok' == strtolower((string)$xml->messages->resultCode) ){
						...
						//issue #1736	
						if(isset($xml->transaction->transactionStatus) && ('declined' == strtolower((string)$xml->transaction->transactionStatus) ||'generalerror' == strtolower((string)$xml->transaction->transactionStatus) ) ){
							$temp_resp['response_status']    = 2;	
							$temp_resp['message_text']       = $xml->transaction->responseReasonDescription;
						}
					}else{
	...
	/**
	 * fetch merchant txn id
	 */
	function _fetch_merchant_txn_id(&$member, $force=false){
		// not set
		if ( ! isset($member->payment_info->txn_id) || empty($member->payment_info->txn_id) || $force) {
			// init
			$authorizenet_transaction_id = 0;

			// check from transaction options table
			if( ! $force ){
				// fetch
				$authorizenet_transaction_id = mgm_get_transaction_option($member->transaction_id, 'authorizenet_transaction_id');
				// log
				mgm_log('fetch from transaction option: '. $authorizenet_transaction_id, __FUNCTION__);
			}			

			// fetch from batch, @todo test before running
			if( ! $authorizenet_transaction_id ){
				// fetch from batch
				$authorizenet_transaction_id = mgm_fetch_authorizenet_missing_txn_id($this, $member->id);

				// log
				mgm_log('fetch from refetch using batch api: '. $authorizenet_transaction_id, __FUNCTION__);
			}
	...
	function get_positional_fields($content){
		//log		    
		mgm_log('status : '.$this->status, __FUNCTION__);					    	
		...
		if (isset($member->payment_info->subscr_id) && !empty($member->payment_info->subscr_id)) {					
			...
			$post_data ='<?xml version="1.0" encoding="utf-8"?>
				<ARBUpdateSubscriptionRequest xmlns="AnetApi/xml/v1/schema/AnetApiSchema.xsd">
				  ...
				  <refId>'.$member->transaction_id.'</refId>
				  <subscriptionId>'.$member->payment_info->subscr_id.'</subscriptionId>
				  <subscription>
				    <payment>
				      <creditCard>
				        <cardNumber>'. mgm_post_var('mgm_card_number', '', true) .'</cardNumber>
				        <expirationDate>' . mgm_post_var('mgm_card_expiry_year', '', true) .'-'. mgm_post_var('mgm_card_expiry_month', '', true) . '</expirationDate>
				      </creditCard>
				    </payment>
				  </subscription>
			...
			if(isset($arb_response['response_status']) && $arb_response['response_status'] == 1){	
				...
				// return to credit card form
				$error_string = sprintf('ARB Error %s: %s - %s',$arb_response['ref_id'], $arb_response['message_code'], $arb_response['message_text']);
				// return
