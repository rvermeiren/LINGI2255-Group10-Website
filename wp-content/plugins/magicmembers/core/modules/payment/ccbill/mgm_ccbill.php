...
class mgm_ccbill extends mgm_payment{	
	...
	function process_return() {
		...
		//debug log
		if(bool_from_yn($this->setting['debug_log'])) mgm_log('IPN : '.mgm_pr($_REQUEST,true),'debug_'.$this->module.'_'.__FUNCTION__ );	
	...
	function process_notify() {
		...
		if ($this->_verify_callback()) {
			...
			//debug log
			if(bool_from_yn($this->setting['debug_log'])) mgm_log('custom : '.mgm_pr($custom,true),'debug_'.$this->module.'_'.__FUNCTION__ );			
	...
	function _buy_post() {
		...
		if(bool_from_yn($this->setting['debug_log'])) {
			mgm_log('custom : '.mgm_pr($custom,true),'debug_'.$this->module.'_'.__FUNCTION__ );		
			mgm_log('_POST : '.mgm_pr($_POST,true),'debug_'.$this->module.'_'.__FUNCTION__ );		
		}
		// find user
		$user = null;
		// check
	...
	function _verify_digest($payment_status){
		...
		if(isset($this->setting['dynamic_pricing']) && $this->setting['dynamic_pricing'] == 'enabled' ) {
			...
			if($resp_digest != $hash ) {
				// error
				mgm_log('Response verification failed: ' . $resp_digest .' ||||| '.$hash); die();
			}
		}
		...
		/*if(isset($this->setting['dynamic_pricing']) && $this->setting['dynamic_pricing'] == 'enabled' ) {
			...
			$hash = md5( (($payment_status == "Processed"?$_POST['subscription_id']:$_POST['denialId'])) . ($payment_status == "Processed"?"1":"0") . $this->setting['md5_hashsalt'] );
			if($resp_digest != $hash ) {
				mgm_log('Response verification failed:' . $resp_digest .'  ||||| '.$hash);
				die();
			}
		}*/
	}
	...
	function _buy_membership() {		
		...
		if(bool_from_yn($this->setting['debug_log'])) {
			mgm_log('custom : '.mgm_pr($custom,true),'debug_'.$this->module.'_'.__FUNCTION__ );		
			mgm_log('_POST : '.mgm_pr($_POST,true),'debug_'.$this->module.'_'.__FUNCTION__ );		
			mgm_log('before member : '.mgm_pr($member,true),'debug_'.$this->module.'_'.__FUNCTION__ );		
		}		
		...
		if((int)$_POST['reasonForDeclineCode'] == 12){
			...
			// exit
				mgm_log('Double process:' . print_r($_POST,1), $this->module . '_transaction_error'); exit();
			}	
		}		
		...
		//debug log
		if(bool_from_yn($this->setting['debug_log'])) mgm_log("Updated member :".mgm_pr($member,true),'debug_'.$this->module.'_'.__FUNCTION__ );		
		// status change event
	...
	function _cancel_membership($user_id, $redirect = false){
		...
		//debug log
		if(bool_from_yn($this->setting['debug_log'])) {
			mgm_log("subs_pack :".mgm_pr($subs_pack,true),'debug_'.$this->module.'_'.__FUNCTION__ );
			mgm_log("member :".mgm_pr($member,true),'debug_'.$this->module.'_'.__FUNCTION__ );
		}	
		// reset payment info
		...
		//debug log
		/*if(bool_from_yn($this->setting['debug_log'])){ 
			mgm_log("Updated member :".mgm_pr($member,true),'debug_'.$this->module.'_'.__FUNCTION__ );
		}*/			
	...
	function query_rebill_status($user_id, $member=NULL) {	
		...
		//debug log
		if(bool_from_yn($this->setting['debug_log'])) mgm_log("Before update member :".mgm_pr($member,true),'debug_'.$this->module.'_'.__FUNCTION__ );	
		// check	
		if (isset($member->payment_info->subscr_id) && !empty($member->payment_info->subscr_id)) {					
			...
			$url = $endpoint . '?' . $query_string;			
			//debug log
			if(bool_from_yn($this->setting['debug_log'])) mgm_log("url :".$url,'debug_'.$this->module.'_'.__FUNCTION__ );			
			// remote get			
			$http_response = mgm_remote_get($url, null, null, false);
			// log
			mgm_log( $http_response, $this->module .'_'. __FUNCTION__);
			//debug log
			if(bool_from_yn($this->setting['debug_log'])) mgm_log("http_response:".mgm_pr($http_response,true),'debug_'.$this->module.'_'.__FUNCTION__ );
			// subs data
			...
			if($xml = @simplexml_load_string($http_response)){				
				//debug log
				if(bool_from_yn($this->setting['debug_log'])) mgm_log("xml:".mgm_pr($xml,true),'debug_'.$this->module.'_'.__FUNCTION__ );				
				// check
	...
	function _verify_callback(){	
		...
		*/
		...
		//debug log
		if(bool_from_yn($this->setting['debug_log'])) {
			if(!$return) mgm_log("Verify callback back failed :".mgm_pr($_POST,true),'debug_'.$this->module.'_'.__FUNCTION__ );	
		}
		// return
		return $return;	
	}	
	...
	function _convert_upgrade_data(){
		global $wpdb;				
		// validate
		if($_POST['clientAccnum'] == $this->setting['client_acccnum'] && $_POST['clientSubacc'] == $this->setting['client_subacc'] && (int)$_POST['reasonForDeclineCode'] == 0){		
			// valid
			$user_id = NULL;
			// by email
			...
			if(!$user_id){			
				...
				//debug log
				if(bool_from_yn($this->setting['debug_log'])) mgm_log("SQL : ".$sql,'debug_'.$this->module.'_'.__FUNCTION__ );				
				// get user id
			...
			//debug log
			if(bool_from_yn($this->setting['debug_log'])) mgm_log("user_id : ".$user_id,'debug_'.$this->module.'_'.__FUNCTION__ );		
			// check
			if($user_id){										
				// row
			...
			//debug log
			if(bool_from_yn($this->setting['debug_log'])) mgm_log("Append custom transaction id to post :".mgm_pr($_POST,true),'debug_'.$this->module.'_'.__FUNCTION__ );
		}else {
			//debug log
			if(bool_from_yn($this->setting['debug_log'])) mgm_log("convert upgrade data failed :".mgm_pr($_POST,true),'debug_'.$this->module.'_'.__FUNCTION__ );	
		}
	}
