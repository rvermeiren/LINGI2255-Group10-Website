...
class mgm_eway extends mgm_payment{
	...
	function process_credit_card(){		
		...
		// log 
		mgm_log( 'Post Data RebillEvent [' . print_r(mgm_filter_cc_details($post_data),true) . ']', __FUNCTION__);	
		...
		// log
		mgm_log( 'Request [' . $post_data['webservice_action'] . ']' . mgm_filter_cc_details($post_string), __FUNCTION__);
...
//		}
			...
			if (isset($this->response['response_status'])) {
				...
				switch ($response_code) {
					...
					case 'Error':
						...
						//log
						mgm_log('Error Response '. mgm_pr($this->response,true),$this->module.'_'.__FUNCTION__);	
						// save
						$member->save();
					break;									
				}	
	...
	function _pre_auth($post_data){
		// log		
		mgm_log('Post Data' . mgm_pr(mgm_filter_cc_details($post_data), true), __FUNCTION__);
		// xmlauth		
		...
		// log
		mgm_log( 'Request [' . $gateway_method . ']' . mgm_filter_cc_details($post_string), __FUNCTION__);	
		...
		// log
		mgm_log( 'Post Data after Date Convertion [' . $gateway_method . ']' . mgm_pr(mgm_filter_cc_details($post_data), true), __FUNCTION__);	
		// OK
		if( $this->response['response_status'] == 1 ) {
	...
	function process_update_card_details () {		
		...
		if (isset($member->payment_info->rebill_customerid) && isset($member->payment_info->subscr_id) ) {			
			...
			if(strtotime($member->payment_info->rebill_start_date) < strtotime($eway_time)){
				//log
				mgm_log('OLD Rebill Start Date : '.$member->payment_info->rebill_start_date,__FUNCTION__);	
				//inti
			...
			//log
			mgm_log('New Rebill Start Date : '.$next_rebill_sdate,__FUNCTION__);					
