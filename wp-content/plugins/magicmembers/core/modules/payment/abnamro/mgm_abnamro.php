 ...
 * @version    3.0
 */ 
class mgm_abnamro extends mgm_payment{	
	// construct
	...
	function mgm_abnamro(){
		...
		// description
		$this->description = __('ABN AMRO is the financial specialist for expatriates in the Netherlands. '.
								'In the same way that expats are conquering new territory, '.
								'our aim is to offer groundbreaking services with tailor-made products, '.
								'along with skilled advisors who speak fluent English.ABN AMRO: '.
								'a trusted partner in a new environment.For more than 25 years, '.
								'ABN AMRO has been the trusted partner for expats, diplomats, '.
								'civil servants and international organizations.', 'mgm');
	...
	function process_return() {							
		...
		// check		
		if($payment_status == 'Authorized' || $payment_status == 'Payment requested' ){
			// query arg
	...
	function _buy_membership() {	
		...
		$acknowledge_user = $this->is_payment_email_sent($_POST['COMPLUS']);
	...
	function _setup_endpoints($end_points = array()){
		// define defaults
		$end_points_default = array('test' => 'https://internetkassa.abnamro.nl/ncol/test/orderstandard.asp',
									'live' => 'https://internetkassa.abnamro.nl/ncol/prod/orderstandard.asp');	
		// merge
