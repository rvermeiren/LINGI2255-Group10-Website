	...
	*/
    ...
    function _removePKCS5Padding($input){
        ...
        if ($padChar > $blockSize) {
			//log
        	mgm_log('Invalid encryption string :  Check for PadChar is less then Block size failed.',$this->module.'_'.__FUNCTION__);
        }
        
        /* Check by padding by character mask */
        if (strspn($input, chr($padChar), strlen($input) - $padChar) != $padChar) {
        	//log
			mgm_log('Invalid encryption string :Check by padding by character mask failed.',$this->module.'_'.__FUNCTION__);
        }
        ...
        if (preg_match('/[[:^print:]]/', $unpadded)){
       		//log
			mgm_log('Invalid encryption string :Check result for printable characters failed.',$this->module.'_'.__FUNCTION__);
        }
        return $unpadded;
    }	
    ...
    function _decryptAes($strIn, $password) {
        ...
        if (!preg_match('/^[0-9a-fA-F]+$/', $hex)) {
        	//log
			mgm_log('Invalid encryption string : check string is malformed failed.',$this->module.'_'.__FUNCTION__);
        }                
        $strIn = pack('H*', $hex);
