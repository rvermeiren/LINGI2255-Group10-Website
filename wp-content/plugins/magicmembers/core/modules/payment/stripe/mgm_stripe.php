...
class mgm_stripe extends mgm_payment{
	...
	function process_credit_card(){			
		...
		// log
		mgm_log('main action: '.$action, __FUNCTION__);
		...
		// log
		mgm_log( 'action: ' . $action.' post_data: ' . mgm_pr(mgm_filter_cc_details($post_data), true).' input_data: ' . mgm_pr($input_data, true), $this->module . '_' . __FUNCTION__);
		
		// to object
		if( $this->response = $this->_get_api_notify($action, $post_data, $input_data) ){
			// dumo
			mgm_log( 'response: '. mgm_pr($this->response, true), $this->module . '_' . __FUNCTION__ );
			// ok
	...
	function query_rebill_status($user_id, $member=NULL) {	
		...
		if ( isset($member->payment_info->cust_id) && !empty($member->payment_info->cust_id) && 
			...
			if( ! preg_match('/^sub_/', $subscr_id) ){
				// check
				mgm_log($subscr_id.' is not a subscription', $this->module . '_' .__FUNCTION__);
				// return
				return false;
			}		
			// check		
	...
	function _notify2post(){
		...
		if( $notify_response = $this->_get_notify() ){
			...
			// log
			mgm_log($notify_response, $type . '_' . $this->module . __FUNCTION__);
			// set notify type
				...
				if( $tran = mgm_get_transaction_by_option('stripe_customer_id', $customer_id) ){
					...
					if( $member = mgm_get_member($tran['data']['user_id']) ){
						...
						if( $process_cancellation && ! in_array($member->status, array( MGM_STATUS_AWAITING_CANCEL, MGM_STATUS_CANCELLED)) ){
							...
							// log
							mgm_log('Cancelling from Notify Post' . mgm_pr($tran, true), $this->module . __FUNCTION__);
						}						
					}					
				}
			}
			...
			if( $notify_response->type == 'customer.subscription.updated'){
				...
				if( $tran = mgm_get_transaction_by_option('stripe_customer_id', $customer_id) ){
					...
					// log
					mgm_log('Updating from Notify Post' . mgm_pr($tran, true), $this->module . __FUNCTION__);
				}
			}
	...
	function _get_api_notify($action, $post_data=null, $input_data=null){
		...
		// log
		mgm_log( 'action: ' . $action . ' post_data: ' . mgm_pr( mgm_filter_cc_details($post_data), true), $this->module . __FUNCTION__);
		// action
		switch(  $action ) {
			case 'create_customer':
			case 'create_charge':
				...
				// log
				mgm_log('action: ' . $action . ' api_url: ' . $api_url . ' http_response: ' . $http_response, $this->module . __FUNCTION__);
				// return
					...
					if( isset($customer->id) ){
						...
						if( $charge = $this->_get_api_notify('create_charge', $post_data_charge) ){
							...
							if( $charge->paid == true ){
								...
								// log
								mgm_log( 'action: ' . $action  .' customer: ' . mgm_pr($customer, true), $this->module . __FUNCTION__);
								// return
								return $customer;							
							}
						}
			...
			case 'use_customer_and_change_subscription':
				...
				if( ! empty($customer_id) && ! empty($subscription_id) ){
					...
					$http_response = mgm_remote_post($api_url, $post_data, array('headers'=>$http_headers, 'timeout'=>30, 'sslverify'=>false), false);
					// log
					mgm_log('action: ' . $action . ' api_url: ' . $api_url . ' post_date: '. mgm_pr(mgm_filter_cc_details($post_data),true) . ' http_response: ' . $http_response, $this->module . __FUNCTION__);
					// return
					if( $notify = json_decode($http_response) ){
						// return
						return $notify;
